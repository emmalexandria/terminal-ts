...
export class Terminal {
  ...
  constructor(vfs: Vfs, outputCallback: OutputCallback, promptCallback: PromptUpdateCallback, promptContentCallback: PromptContentCallback) {
  ...
  print(input: string) {
    const output = { text: input, fg: OutputColor.DEFAULT, bg: OutputColor.DEFAULT, attributes: [] }
  ...
  input(input: string) {
    ...
    if (command) {
      ...
      this.promptUpdateCallback({ vfs: this.vfs, currentCommand: command.name })
      return
    } else {
      this.printColored({ text: `No command found for ${input}`, fg: OutputColor.RED, bg: OutputColor.DEFAULT, attributes: [] })
    }
  }
