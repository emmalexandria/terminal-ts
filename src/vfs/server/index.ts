import { FileType, FSEntry, VirtualDirectory, VirtualFile, VirtualSymlink, FSRoot } from "vfs/fs.js";
import path, { relative } from "path";
import fs, { Dirent, Stats } from "fs"

export const buildVFS = async (root: string): Promise<VirtualDirectory> => {
  const VFS = await walkDir(root)
...
const walkDir = async (dir: string): Promise<VirtualDirectory> => {
  ...
  for (const entry of entries) {
    ...
    if (fsEntry.type == "file") {
      const file: VirtualFile = { ...fsEntry, content: await fs.promises.readFile(fullPath), type: "file" }
      root.entries[entry.name] = file
    }
    if (fsEntry.type == "symlink") {
      const symlink: VirtualSymlink = { ...fsEntry, target: await fs.promises.readlink(fullPath), type: "symlink" }
...
const makeDirectoryRelative = (dir: VirtualDirectory) => {
  ...
  const makeChildrenRelative = (currentDir: VirtualDirectory) => {
    for (const key of Object.keys(currentDir.entries)) {
      currentDir.entries[key].path = path.relative(currentDir.path, currentDir.entries[key].path)
    }
  }
...
export const vfsDirectoryFromPath = async (fullPath: string): Promise<VirtualDirectory> => {
  const stat = await fs.promises.stat(fullPath)
